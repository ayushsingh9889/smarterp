            ...
            <div className="bg-white p-6 rounded shadow mb-6">
                <div className="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" placeholder="Expense/Asset" value={form.debit_account}
                    ...
                    <div>
                        ...
                        <input type="text" placeholder="Income/Liability" value={form.credit_account}
                    ...
                    <div>
                        ...
                        <input type="text" placeholder="Depreciation, Provision..." value={form.description}
            ...
            <div className="bg-white rounded shadow overflow-hidden">
                ...
                <table className="w-full">
                    <thead className="bg-gray-50"><tr>
                        ...
                        <th className="text-left px-6 py-2 text-xs">Debit</th>
                        <th className="text-left px-6 py-2 text-xs">Credit</th>
                        <th className="text-left px-6 py-2 text-xs">Amount</th>
                        <th className="text-left px-6 py-2 text-xs">Desc</th>
                        <th className="text-left px-6 py-2 text-xs">Date</th>
                    </tr></thead>
                    <tbody>
                        {journals.map(j => (
                            <tr key={j.id}>
                                <td className="px-6 py-2 border-b text-sm">{j.id}</td>
                                <td className="px-6 py-2 border-b text-sm">{j.debit_account}</td>
                                <td className="px-6 py-2 border-b text-sm">{j.credit_account}</td>
                                <td className="px-6 py-2 border-b text-sm font-medium">₹{j.amount}</td>
                                <td className="px-6 py-2 border-b text-sm">{j.description}</td>
                                <td className="px-6 py-2 border-b text-sm">{new Date(j.created_at).toLocaleDateString()}</td>
                            </tr>
                        ))}
                    </tbody>
                </table>
