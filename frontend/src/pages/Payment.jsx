    ...
    <div>
      ...
      <div className="bg-white p-6 rounded shadow mb-6">
        <div className="grid grid-cols-2 gap-4">
          <div><label className="block text-sm font-medium mb-1">Paid To *</label><input type="text" value={form.paid_to} onChange={(e) => setForm({ ...form, paid_to: e.target.value })} className="w-full border border-gray-300 rounded px-3 py-2" /></div>
      ...
      <div className="bg-white rounded shadow overflow-hidden">
        ...
        <table className="w-full"><thead className="bg-gray-50"><tr><th className="text-left px-6 py-2 text-xs">ID</th><th className="text-left px-6 py-2 text-xs">Paid To</th><th className="text-left px-6 py-2 text-xs">Amount</th><th className="text-left px-6 py-2 text-xs">Mode</th><th className="text-left px-6 py-2 text-xs">Desc</th><th className="text-left px-6 py-2 text-xs">Date</th></tr></thead>
          <tbody>{payments.map(p => (<tr key={p.id}><td className="px-6 py-2 border-b text-sm">{p.id}</td><td className="px-6 py-2 border-b text-sm">{p.paid_to}</td><td className="px-6 py-2 border-b text-sm font-medium text-red-600">₹{p.amount}</td><td className="px-6 py-2 border-b text-sm">{p.payment_mode}</td><td className="px-6 py-2 border-b text-sm">{p.description}</td><td className="px-6 py-2 border-b text-sm">{new Date(p.created_at).toLocaleDateString()}</td></tr>))}</tbody></table>
      </div>
    </div>
  )
}
