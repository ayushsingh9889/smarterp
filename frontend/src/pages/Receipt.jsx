...
function Receipt() {
    ...
    return (
        <div>
            ...
            <p className="text-gray-500 mb-4 text-sm">Record money received (Sales, Payments etc.)</p>

            <div className="bg-white p-6 rounded shadow mb-6">
                <div className="grid grid-cols-2 gap-4">
                    <div>
                        <label className="block text-sm font-medium mb-1">Received From *</label>
                        <input type="text" placeholder="Name/Company" value={form.received_from}
                    ...
                    <div>
                        ...
                        <input type="text" placeholder="Sales, Commission, etc." value={form.description}
                ...
                <table className="w-full">
                    <thead className="bg-gray-50">
                        <tr>
                            ...
                            <th className="text-left px-6 py-2 text-xs">Received From</th>
                    ...
                    <tbody>
                        {receipts.map(r => (
                            <tr key={r.id}>
                                <td className="px-6 py-2 border-b text-sm">{r.id}</td>
                                <td className="px-6 py-2 border-b text-sm">{r.received_from}</td>
                                <td className="px-6 py-2 border-b text-sm font-medium text-green-600">₹{r.amount}</td>
                                <td className="px-6 py-2 border-b text-sm">{r.payment_mode}</td>
                                <td className="px-6 py-2 border-b text-sm">{r.description}</td>
                                <td className="px-6 py-2 border-b text-sm">{new Date(r.created_at).toLocaleDateString()}</td>
                            </tr>
                        ))}
                    </tbody>
                </table>
